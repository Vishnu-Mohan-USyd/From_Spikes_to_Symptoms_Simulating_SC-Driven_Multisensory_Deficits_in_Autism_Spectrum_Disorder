...
def load_ckpt(ckpt_path, seed, B, device):
    ...
    # task#385 SOM/Martinotti facilitation-in-time: restore the 3 trained scalars from mh (ckpt = source of
    # truth -> MEASURE honors the TRAIN-time facilitation, mirroring tau_gaba/gNMDA). F_gaba itself is a
    # per-trial transient rebuilt by reset_state at the restored U (NOT in state_dict), so no state_dict change.
    if "facil_gaba_on"  in mh: net.facil_gaba_on  = int(mh["facil_gaba_on"])
    if "tau_facil_gaba" in mh: net.tau_facil_gaba = float(mh["tau_facil_gaba"])
    if "U_facil_gaba"   in mh: net.U_facil_gaba   = float(mh["U_facil_gaba"])
    ...
    # ASD lever (MEASURE-side, mirrors g_GABA #94): pv_gaba_scale scales the disynaptic PV->MSI_exc GABA.
    # saved to mutable_hparams by both builds; restore the TRAINED value so an ASD retrain is graded at its
    # trained inhibition. Existing NT ckpts (no pv_gaba_scale) keep the build default 1.0 => byte-identical.
    if "pv_gaba_scale" in mh:
        net.pv_gaba_scale = float(mh["pv_gaba_scale"])
    _env_pv = os.environ.get("PV_GABA_SCALE")
    if _env_pv is not None:
        assert abs(float(net.pv_gaba_scale) - float(_env_pv)) < 1e-9, \
            f"FATAL: ckpt pv_gaba_scale={float(net.pv_gaba_scale)} != PV_GABA_SCALE env {_env_pv}"
    ...
    # task#327 (MEASURE-side, mirrors g_GABA/v2msi): afferent_jitter_ms is the per-channel INDEPENDENT afferent-
    # arrival (conduction-delay) jitter σ (ms), saved to mutable_hparams + env-exposed (AFFERENT_JITTER_MS) in both
    # builds. Restore the TRAINED value (CKPT = source of truth) so the eager measurement forward applies the SAME
    # arrival jitter it was trained with; AFFERENT_JITTER_MS env (if set) must agree. Old ckpts (no mh entry) restore
    # to the build default 0.0 => byte-identical / no jitter.
    if "afferent_jitter_ms" in mh:
        net.afferent_jitter_ms = float(mh["afferent_jitter_ms"])
    else:
        print(f"[load_ckpt] WARN: ckpt has no mutable_hparams['afferent_jitter_ms'] -> falls back to "
              f"build default {float(net.afferent_jitter_ms)}", flush=True)
    _env_affjit = os.environ.get("AFFERENT_JITTER_MS")
    if _env_affjit is not None:
        assert abs(float(net.afferent_jitter_ms) - float(_env_affjit)) < 1e-9, \
            f"FATAL: ckpt afferent_jitter_ms={float(net.afferent_jitter_ms)} != AFFERENT_JITTER_MS env {_env_affjit}"
    ...
    for _mhk, _attr, _envn in [("tau_rec", "tau_rec", "TAU_REC"),
                               ...
                               ("bM", "bM", "ADAPT_BM"),
                               ("dM", "dM", "ADAPT_DM"),
                               ("aM", "aM", "ADAPT_A"),          # task#288 recovery-timescale lever (tau_adapt=1/aM); restore TRAINED value so MEASURE matches training
                               ("aMi", "aMi", "ADAPT_A_INH"),    # task#376 interneuron SFA recovery-rate (tau_adapt_inh=1/aMi); restore TRAINED value so MEASURE honors sustained inh adaptation
                               ("dMi", "dMi", "ADAPT_DM_INH"),   # task#376 interneuron spike-triggered adaptation increment; restore TRAINED value
