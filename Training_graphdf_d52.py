...
def generate_event_loc_seq_batch(batch_size=32,
    ...
    temporal_jitter_max : int
        Max A/V onset offset in frames (0 disables jitter). Legacy per-EVENT INDEPENDENT
        onset jitter — left untouched for byte-identity.
    sigma_dL_frames : float
        task #100 — std (in 10 ms macro-frames) of the per-trial COMMON-MODE A-V latency
        jitter ΔL. ONE shared Gaussian draw per trial shifts the whole trial's A-V relative
        onset together (correlated, unlike temporal_jitter_max). 0.0 disables it AND draws
        nothing -> numpy RNG stream untouched -> byte-identical to the pristine build.
    """
    ...
    for _ in range(batch_size):
        ...
        while t <= T - D:
            if np.random.rand() < p_start:
                ...
                # (shared dL_trial across the whole trial; integer-frame; matches the measurement-path
                # edit + the #98-H4 kernel). Clamped to the in-window room so BOTH bursts are always
                # placed (no lost modality, no truncation bias). dL_trial == 0 -> rel == dt_frames ->
...
class MultiBatchAudVisMSINetworkTime(nn.Module):
    ...
    def __init__(
        ...
        # task #100 — correlated trial-timing noise (common-mode A-V latency jitter ΔL) baked into
        # TRAINING for the graded-bell fix. Gated net-level knob, DEFAULT 0.0 -> byte-identical to the
        # pristine fast build (generator md5 f0093eef). Units: macro-step frames (10 ms each); 3.0 = 30 ms.
        # Consumed EAGERLY per minibatch by generate_event_loc_seq_batch; the L6 graph captures only the
        # substep dynamics and the jittered xA/xV are copied into the static input buffers EVERY replay
        # (_l6_forward: sxA.copy_(xA_b)), so the mechanism is live through the graph (never frozen).
        ...
        # TRAIN-side twin of the delayfix (MEASURE) knob: per-CHANNEL (A,V independent), per-TRIAL,
        # SUBSTEP-resolution arrival jitter on the A/V conduction ring-buffer READ, baked into training.
        # GATED, DEFAULT 0.0 -> no draw (RNG untouched) -> buffers unchanged (length==delay) -> the read
        # keeps the exact index_select/buf[pos] expression -> BYTE-IDENTICAL to the pristine graphed build.
        # The persistent per-trial offset tensors are LONG (like the _gpos cursors), so they are excluded
        # from the float graph-reset pool and re-drawn IN-PLACE each reset (static address = CUDA-graph-safe).
        ...
        # CUDA-graph note: the `if self._formA_on` branch is resolved at capture (a fixed Python bool) -> the captured
        # graph carries the static rule when OFF (byte-identical) or the adaptive rule when ON; the trace update is pure
        # elementwise (graph-safe, no data-dependent control flow). Baked into training => auto-present in train+measure.
        ...
        self.tau_gaba = float(os.environ.get('TAU_GABA', '50.0'))   # ms — task#P6 env override (default 50 = biology baseline; read live into gaba_decay each substep)
        self.pv_gaba_scale = float(os.environ.get('PV_GABA_SCALE', '1.0'))  # ASD lever: multiplicative scale on the disynaptic PV(MSI_inh)->MSI_exc GABA conductance. 1.0 = NT byte-identical (x1.0 exact); <1 = reduced PV feed-forward inhibition, BAKED into training + saved to mutable_hparams so val36.load_ckpt restores it => measurement matches training.
        # New training uses the calibrated GABA-A conductance equation. The
        # legacy current equation remains selectable only for old-checkpoint
        # regression and is recorded explicitly in every new checkpoint.
        self.set_local_gaba_configuration(
            os.environ.get('LOCAL_GABA_MODE', LOCAL_GABA_MODE_CONDUCTANCE)
        )
        ...
        # Accumulates local disynaptic GABA activation (MSI_inh->MSI_exc).
        # Conductance mode converts it to g*s*(V-E_GABA) at integration.
        # Legacy mode continues to share this state with subtractive surround.
        self.I_M_gaba = torch.zeros((self.batch_size, self.n), device=self.device)
        # Conductance-mode local GABA cannot share a state with the unchanged
        # current-based lateral surround; this accumulator keeps the mechanisms
        # separate without changing surround inhibition itself.
        ...
        self.conduction_delay_msi_inh2exc = 52   # task TBW-delay-fix: local disynaptic IPSC ~5.2ms (Whyland-Bickford 2018); corrects the cat-SC sensory-leg conflation (was 450=45ms)
    ...
    def _ensure_delay_buffer(self, attr: str, *, delay: int, width: int, extra: int = 0) -> None:
        ...
        extra==0 (default) => length==delay => byte-identical to the pristine build.
        """
    ...
    def _reset_delay_buffers(self) -> None:
        ...
        # margin = ceil(5σ) substeps, capped at delay_base-1 (symmetric ±margin clip stays mean-preserving
    ...
    def _draw_aff_jitter(self) -> None:
        """task #327: (re)draw the per-trial, per-channel INDEPENDENT afferent-arrival jitter offsets.

        GATED: OFF (afferent_jitter_ms<=0) => _aff_jitter_* = None (forward keeps the pristine
        index_select/buf[pos] read => byte-identical). ON => two independent (batch,) integer substep
        offsets, σ = afferent_jitter_ms/dt, symmetric-clipped to ±margin (mean-preserving).

        Graph-safety: the offset tensors are LONG (excluded from the float graph-reset pool) and held in a
        PER-BATCH-SIZE persistent pool — created once, then filled IN-PLACE (static address) — so the captured
        B=main graph reads its own tensor bit-stably even across the eager B=tail batch (task #34 two-pool)."""
        if float(getattr(self, 'afferent_jitter_ms', 0.0)) <= 0.0:
            self._aff_jitter_a = None
            self._aff_jitter_v = None
            return
        B = self.batch_size
        _sig = float(self.afferent_jitter_ms) / float(self.dt)
        _ma = int(self._aff_margin_a); _mv = int(self._aff_margin_v)
        _ja = torch.round(torch.randn(B, device=self.device) * _sig).long().clamp_(-_ma, _ma)
        _jv = torch.round(torch.randn(B, device=self.device) * _sig).long().clamp_(-_mv, _mv)
        pools = getattr(self, '_aff_jitter_pools', None)
        if pools is None:
            pools = self._aff_jitter_pools = {}
        p = pools.get(B)
        if p is None:
            pools[B] = {'a': _ja, 'v': _jv, 'arange': torch.arange(B, device=self.device)}
        else:
            p['a'].copy_(_ja)      # IN-PLACE fill -> address static -> CUDA-graph reads fresh per-trial values
            p['v'].copy_(_jv)
        p = pools[B]
        self._aff_jitter_a = p['a']
        self._aff_jitter_v = p['v']
        self._aff_batch_arange = p['arange']
    ...
    def reset_state(self, batch_size=None):
        ...
        if _gc is not None:
            ...
            self._draw_aff_jitter()   # task #327: re-draw per-trial arrival jitter IN-PLACE (graph-safe)
        ...
        # task #327: (create-once, then fill-in-place) the per-trial arrival jitter for THIS batch size.
        # Runs for BOTH the _l4 graphed first-reset (after the gpos sync above) and any non-graph eager reset;
        # per-batch persistence keeps the captured graph's baked address valid across the eager tail batch.
        self._draw_aff_jitter()
    ...
    def update_all_layers_batch(self,
        ...
        separate_subtractive_surround = bool(
            ...
            and self.local_gaba_mode == LOCAL_GABA_MODE_CONDUCTANCE
        )
        ...
        for sub_i in range(self.n_substeps):
            ...
            if self.gaba_shunt_surr:
                self.I_M_gaba_surr_sh.mul_(gaba_decay)
            elif separate_subtractive_surround:
                self.I_M_gaba_surr_sub.mul_(gaba_decay)
            ...
            if _l4:  # LEVER L4: GPU-cursor gather (index read at exec time)
                # task #327 — OFF (jitter None): exact pristine index_select (byte-identical). ON: capture-safe
                # per-element gather at (gpos - delay_base - jitter_b) mod L; +jitter_b = older row = longer delay.
                if self._aff_jitter_a is None:
                    ...
                    _iA = (gpos_a2msi - delay_a2msi - self._aff_jitter_a).remainder(buf_a2msi.shape[0])
                    delayed_spikes_a2msi = buf_a2msi.gather(0, _iA.view(1, -1, 1).expand(1, -1, self.n)).squeeze(0)
                else:
                ...
                if self._aff_jitter_v is None:
                    ...
                    _iV = (gpos_v2msi - delay_v2msi - self._aff_jitter_v).remainder(buf_v2msi.shape[0])
                    delayed_spikes_v2msi = buf_v2msi.gather(0, _iV.view(1, -1, 1).expand(1, -1, self.n)).squeeze(0)
                else:
                ...
                # task #327 — OFF (jitter None): exact pristine buf[pos] view (byte-identical). ON: per-element
                # gather (a COPY) at (pos - delay_base - jitter_b) mod L, mirroring the graphed path.
                if self._aff_jitter_a is None:
                    delayed_spikes_a2msi = buf_a2msi[pos_a2msi] if delay_a2msi > 0 else zero_exc
                elif delay_a2msi > 0:
                    _iA = (pos_a2msi - delay_a2msi - self._aff_jitter_a) % buf_a2msi.shape[0]
                    delayed_spikes_a2msi = buf_a2msi[_iA, self._aff_batch_arange]
                else:
                    delayed_spikes_a2msi = zero_exc
                if self._aff_jitter_v is None:
                    delayed_spikes_v2msi = buf_v2msi[pos_v2msi] if delay_v2msi > 0 else zero_exc
                elif delay_v2msi > 0:
                    _iV = (pos_v2msi - delay_v2msi - self._aff_jitter_v) % buf_v2msi.shape[0]
                    delayed_spikes_v2msi = buf_v2msi[_iV, self._aff_batch_arange]
                else:
                    delayed_spikes_v2msi = zero_exc
            ...
            self.I_M_gaba.add_(self.pv_gaba_scale * I_M_inh2exc)   # PV_GABA_SCALE lever (default 1.0 => x1.0 bit-exact; ASD retrain sets <1 to reduce the disynaptic PV feed-forward inhibition, baked in)
            ...
            if self.gaba_shunt_surr:
                ...
                self.I_M_gaba_surr_sub.add_(self.g_GABA * I_latM)  # unchanged subtractive surround, separate from local conductance
            else:
            ...
            # task#385 SOM/Martinotti facilitation-in-time on the interneuron->exc GABA edge (debugger #384 §2B.3).
            ...
            if _l4:  # LEVER L4: GPU-cursor scatter + in-graph modular advance
                if delay_a2msi > 0:
                    ...
                    gpos_a2msi.add_(1).remainder_(delay_a2msi if self._aff_jitter_a is None else buf_a2msi.shape[0])  # task #327: mod ring length when jittering
                ...
                if delay_v2msi > 0:
                    ...
                    gpos_v2msi.add_(1).remainder_(delay_v2msi if self._aff_jitter_v is None else buf_v2msi.shape[0])  # task #327
                ...
                if delay_msi_inh2exc > 0:
                    buf_msi_inh2exc.index_copy_(0, gpos_msi_inh2exc, rel_sMi.unsqueeze(0))  # task#385: facilitated release (OFF: rel_sMi==new_sMi)
                ...
                if delay_a2msi > 0:
                    ...
                    pos_a2msi = (pos_a2msi + 1) % (delay_a2msi if self._aff_jitter_a is None else buf_a2msi.shape[0])  # task #327
                    if self._aff_jitter_a is not None:
                        delayed_spikes_a2msi = new_sA  # task #327 SHIP-PARITY: ON read is a copy (no view-clobber); re-bind for STDP
                if delay_v2msi > 0:
                    buf_v2msi[pos_v2msi].copy_(new_sV)
                    pos_v2msi = (pos_v2msi + 1) % (delay_v2msi if self._aff_jitter_v is None else buf_v2msi.shape[0])  # task #327
                    if self._aff_jitter_v is not None:
                        delayed_spikes_v2msi = new_sV  # task #327 SHIP-PARITY
                if delay_a2msi_inh > 0:
    ...
    def train_unsupervised_batch(self,
        ...
        while seq_counter < n_sequences:
            ...
            loc_seqs, mod_seqs, offset_ok, seq_lens = generate_event_loc_seq_batch(
                ...
                # task #100 — INTRINSIC common-mode A-V latency jitter in TRAINING (same knob/mechanism
                # as the measurement path). self.sigma_dL_frames == 0.0 -> byte-identical to pristine.
...
def make_checkpoint(net,
    ...
    mutable_hparams = dict(
        ...
        pv_gaba_scale=net.pv_gaba_scale,   # ASD lever: disynaptic PV->exc GABA scale (source of truth; val36.load_ckpt restores => measurement matches training)
        ...
        afferent_jitter_ms=net.afferent_jitter_ms,   # task#327: per-channel afferent-arrival jitter σ (ms); source of truth so val36.load_ckpt restores it => MEASURE honors the trained jitter
