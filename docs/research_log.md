...
`ecadfd4d472cead684c2d49d18657df4c1b6525b`. Final validation was performed on
2026-07-26. This document and the versioned
...
```bash
env CUDA_VISIBLE_DEVICES=1 MPLBACKEND=Agg PYTHONUNBUFFERED=1 python -m pytest -q -s
...
All eight commands exited 0. Their exact accepted results are recorded in the
matrix below. The focused tests reported `6 passed in 273.21s (0:04:33)`; the
...
|---|---|---|---|
| Test and device gate | 6 tests passed in 273.21 s; physical GPU1 exposed as logical CUDA 0, NVIDIA RTX A6000 | Pass | `python -m pytest -q -s` |
...
| Fano | Primary active-ROI FF `1.123541 → .783592` in the first 100 ms; quenching 10/10; mean rate `.095922 → .121753`, rising 8/10 | Pass: variability quenching | `python fano_factor_test.py` and `python -m pytest -q -s` |
