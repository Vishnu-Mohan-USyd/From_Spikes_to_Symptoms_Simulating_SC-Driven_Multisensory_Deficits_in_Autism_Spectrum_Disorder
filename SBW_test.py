...
def spatial_binding_diagnostics(
    ...
    def is_fused(profile: np.ndarray) -> bool:
        ...
        if sm.max() < 1e-6:  # silent — no spatial representation
            return False
        sm /= sm.max()
        peaks, props = find_peaks(sm, height=0.5, distance=10)
        if len(peaks) <= 1:
            return True
...
def spatial_binding_curve_fast(
    ...
    for k, sep in enumerate(separations_deg):
        ...
        locA_deg = base_deg
        locV_deg = _second_stim_loc(base_deg, sep, S, linear=linear)
...
def compute_sbw_fused_persep(
    ...
    def is_fused(profile: np.ndarray) -> bool:
        sm = gaussian_filter1d(profile, sigma=2, mode=("nearest" if linear else "wrap"))
        if sm.max() < 1e-6:  # silent — no spatial representation
            return False
        sm /= sm.max()
        peaks, props = find_peaks(sm, height=0.5, distance=10)
...
def compute_sbw_enhancement_persep(
    ...
    def roi_spikes(msi_sum, center_idx):
        """Sum spikes in ±roi_half neurons around center_idx.

        linear=True clips the ROI to [0, N-1] (LINEAR map); legacy wraps (% N).
        """
        offsets = torch.arange(-roi_half, roi_half + 1, device=msi_sum.device)
        idx_raw = center_idx[:, None] + offsets[None, :]
        indices = idx_raw.clamp(0, N - 1) if linear else (idx_raw % N)
    ...
    for k, sep in enumerate(separations_deg):
        ...
        all_locA[s:e] = base
        all_locV[s:e] = _second_stim_loc(base, sep, S, linear=linear)
...
def _second_stim_loc(base, sep, S, *, linear=False):
    """Locate the 2nd (V) stimulus from base (A) loc + signed separation.

    linear=False (LEGACY): circular wrap ``(base + sep) % S`` — assumes a neuron
        *ring*; a trial with base+sep>S-1 wraps to the low end so its TRUE on-map
        separation becomes S-|sep| (mislabelled → biases the SBW half-width narrow).
    linear=True: the trained map is a LINEAR bounded segment [0, S-1]; place V so the
        on-map distance equals |sep| EXACTLY by reflecting at the boundary (no '%').
        Mirrors the topographic linear fix.
    """
    base = np.asarray(base)
    lv = base + sep
    if not linear:
        return lv % S
    lv = np.where((lv < 0) | (lv > S - 1), base - sep, lv)
    return np.clip(lv, 0, S - 1)
...
def run_spatial_binding_across_models(
    ...
    if method in ("enhancement", "is_fused"):
        ...
        # Old normalization path for spike_profile method only
